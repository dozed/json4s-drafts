...
trait OAuthJSONR extends OAuthTypes {
  ...
  implicit val tokenResponseRead = JSONR.instanceE[TokenResponse] { jv =>
    if ((jv \ "error").isEmpty) {
  ...
  implicit val tokenResponseErrorRead = JSONR.instance[ErrorCode.TokenResponseError] { json =>

    val facebookError = JSONR.instance[ErrorCode.TokenResponseError] { json =>
      ((json \ "error" \ "code").validate[String] |@| (json \ "error" \ "message").validate[String]) (ErrorCode.TokenResponseError)
    }

    val googleError = JSONR.instance[ErrorCode.TokenResponseError] { json =>
